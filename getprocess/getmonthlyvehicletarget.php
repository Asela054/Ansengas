    ...
    <tbody>
        ...
        <tr>
            ...
            foreach($productarray as $rowproductarray){
                $vehicleID = $row['idtbl_vehicle'];
                $productID = $rowproductarray->productID;
                $sqlavaqty = "SELECT SUM(`targettank`) AS `targettank`,SUM(`targetcomplete`) AS `targetcomplete` FROM `tbl_vehicle_target` WHERE `status` = 1 AND `tbl_vehicle_idtbl_vehicle` = '$vehicleID' AND `tbl_product_idtbl_product` = '$productID' AND MONTH(`month`) = MONTH(CURRENT_DATE()) AND YEAR(`month`) = YEAR(CURRENT_DATE())";
                $resultavaqty = $conn->query($sqlavaqty);
                $rowavaqty = $resultavaqty->fetch_assoc();
            ?>
