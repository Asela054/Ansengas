        ...
        <table class="table table-striped table-bordered table-sm nowrap w-100" id="tableunloading">
            ...
            <tbody>
                ...
                while($row=$result->fetch_assoc()){ 
                    ...
                    $sqlinvoiceqty="SELECT SUM(`newqty`+`refillqty`+`emptyqty`) AS `qty`, SUM(`refillqty`) AS `refillqty`, SUM(`newqty`) AS `newqty`, SUM(`emptyqty`) AS `emptyqty` FROM `tbl_invoice_detail` WHERE `tbl_product_idtbl_product`='$productID' AND `status`=1 AND `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `status`=1 AND `tbl_vehicle_load_idtbl_vehicle_load`='$loadID')";
