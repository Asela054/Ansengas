...
if (!empty($_POST['invoiceno'])) {
    ...
    if (strpos($invoiceno, '-') !== false) {
        ...
        $invoiceID = substr($invoiceno, 3);
        // $sql = "SELECT `tbl_invoice`.`idtbl_invoice`, `tbl_invoice`.`tax_invoice_num`, `tbl_invoice`.`non_tax_invoice_num`, `tbl_invoice`.`date`, `tbl_invoice`.`nettotal`, SUM(`tbl_invoice_payment_has_tbl_invoice`.`payamount`) AS `payamount`,`tbl_invoice`.`tbl_customer_idtbl_customer` FROM `tbl_invoice` LEFT JOIN `tbl_invoice_payment_has_tbl_invoice` ON `tbl_invoice_payment_has_tbl_invoice`.`tbl_invoice_idtbl_invoice` = `tbl_invoice`.`idtbl_invoice` WHERE `tbl_invoice`.`idtbl_invoice`='$invoiceID' AND `tbl_invoice`.`status`=1 AND `tbl_invoice`.`paymentcomplete`=0 GROUP BY `tbl_invoice`.`idtbl_invoice`";
        $sql = "SELECT * FROM (SELECT `idtbl_invoice`, `tax_invoice_num`, `non_tax_invoice_num`, `date`, `nettotal`, `tbl_customer_idtbl_customer` FROM `tbl_invoice` WHERE `idtbl_invoice`='$invoiceID' AND `status`=1 AND `paymentcomplete`=0) AS `dinv` LEFT JOIN (SELECT SUM(`payamount`) AS `payamount`, `tbl_invoice_idtbl_invoice` FROM `tbl_invoice_payment_has_tbl_invoice` GROUP BY `tbl_invoice_idtbl_invoice`) AS `dhasinv` ON `dhasinv`.`tbl_invoice_idtbl_invoice`=`dinv`.`idtbl_invoice` ORDER BY `dinv`.`date` DESC";
    } else {
        $invoiceID = substr($invoiceno, 3);
        // $sql = "SELECT `tbl_invoice`.`idtbl_invoice`, `tbl_invoice`.`tax_invoice_num`, `tbl_invoice`.`non_tax_invoice_num`, `tbl_invoice`.`date`, `tbl_invoice`.`nettotal`, SUM(`tbl_invoice_payment_has_tbl_invoice`.`payamount`) AS `payamount`,`tbl_invoice`.`tbl_customer_idtbl_customer` FROM `tbl_invoice` LEFT JOIN `tbl_invoice_payment_has_tbl_invoice` ON `tbl_invoice_payment_has_tbl_invoice`.`tbl_invoice_idtbl_invoice` = `tbl_invoice`.`idtbl_invoice` WHERE `tbl_invoice`.`tax_invoice_num`='$invoiceID' AND `tbl_invoice`.`status`=1 AND `tbl_invoice`.`paymentcomplete`=0 GROUP BY `tbl_invoice`.`idtbl_invoice`";
        $sql = "SELECT * FROM (SELECT `idtbl_invoice`, `tax_invoice_num`, `non_tax_invoice_num`, `date`, `nettotal`, `tbl_customer_idtbl_customer` FROM `tbl_invoice` WHERE `tax_invoice_num`='$invoiceID' AND `status`=1 AND `paymentcomplete`=0) AS `dinv` LEFT JOIN (SELECT SUM(`payamount`) AS `payamount`, `tbl_invoice_idtbl_invoice` FROM `tbl_invoice_payment_has_tbl_invoice` GROUP BY `tbl_invoice_idtbl_invoice`) AS `dhasinv` ON `dhasinv`.`tbl_invoice_idtbl_invoice`=`dinv`.`idtbl_invoice` ORDER BY `dinv`.`date` DESC";
    }
