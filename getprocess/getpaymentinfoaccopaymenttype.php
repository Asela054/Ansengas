...
if($receipttype==1){
    ...
    $sql="SELECT `idtbl_invoice`, `total`, `protoaccountstatus` FROM `tbl_invoice` WHERE `status`=1 AND `paymentcomplete`=0 AND `date` BETWEEN '$validfrom' AND '$validto'";
    $result=$conn->query($sql);
}
?>
<table class="table table-striped table-bordered table-sm" id="paymentlisttable">
    ...
    <tbody>
        ...
        ?>
        <tr class="<?php if($row['protoaccountstatus']==1){echo "table-orange";} ?>">
            <td><?php echo $i; ?></td>
