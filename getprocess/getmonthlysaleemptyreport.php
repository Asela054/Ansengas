...
foreach ($allMonths as $monthYear) {
    $sql .= ",
    SUM(CASE WHEN DATE_FORMAT(i.date, '%Y-%m') = '$monthYear' AND p.idtbl_product = '6' THEN (COALESCE(id.newqty, 0) + COALESCE(id.emptyqty, 0)) ELSE 0 END) AS '{$monthYear}_2KG',
    SUM(CASE WHEN DATE_FORMAT(i.date, '%Y-%m') = '$monthYear' AND p.idtbl_product = '4' THEN (COALESCE(id.newqty, 0) + COALESCE(id.emptyqty, 0)) ELSE 0 END) AS '{$monthYear}_5KG',
    SUM(CASE WHEN DATE_FORMAT(i.date, '%Y-%m') = '$monthYear' AND p.idtbl_product = '1' THEN (COALESCE(id.newqty, 0) + COALESCE(id.emptyqty, 0)) ELSE 0 END) AS '{$monthYear}_12.5KG',
    SUM(CASE WHEN DATE_FORMAT(i.date, '%Y-%m') = '$monthYear' AND p.idtbl_product = '2' THEN (COALESCE(id.newqty, 0) + COALESCE(id.emptyqty, 0)) ELSE 0 END) AS '{$monthYear}_37.5KG'";
}
