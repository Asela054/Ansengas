...
foreach ($period as $key => $value) {
    ...
    foreach($productarray as $rowproductarray){ 
        ...
        $sqlsalecount="SELECT SUM(`newqty`) AS `newqty`, SUM(`refillqty`) AS `refillqty`, SUM(`emptyqty`) AS `emptyqty` FROM `tbl_invoice_detail` WHERE `status`=1 AND `tbl_product_idtbl_product`='$productID' AND `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `status`=1 AND `date`='$days')";
        ...
        if(!empty($rowsalecount['emptyqty'])){
            $trustsalecount=$rowsalecount['emptyqty'];
        }
...
$html.='<table class="table table-striped table-bordered table-sm small" id="tableproductsale">
    ...
    <tbody>';
        foreach($productarray as $rowproductarray){ 
            ...
            $sqlsalecount="SELECT SUM(`newqty`) AS `newqty`, SUM(`refillqty`) AS `refillqty`, SUM(`emptyqty`) AS `emptyqty` FROM `tbl_invoice_detail` WHERE `status`=1 AND `tbl_product_idtbl_product`='$productID' AND `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `status`=1 AND `date` BETWEEN '$validfrom' AND '$validto')";
            $resultsalecount =$conn-> query($sqlsalecount);
            $rowsalecount = $resultsalecount-> fetch_assoc();
        $html.='<tr>
            ...
            <td class="text-center">'.$rowsalecount['emptyqty'].'</td>
        </tr>';
        }
    $html.='</tbody>
</table>';
