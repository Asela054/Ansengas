    ...
    <tbody>
        ...
        while($rowcustomer = $resultcustomer-> fetch_assoc()){ 
            ...
            if($validfrom=='' && $validto==''){
                $sqlinvoicelist="SELECT SUM(CASE WHEN `tbl_invoice_detail`.`tbl_product_idtbl_product` = 1 THEN `tbl_invoice_detail`.`refillqty` ELSE 0 END) AS refillqty, SUM(`tbl_invoice`.`nettotal`) AS nettotal,`tbl_invoice_detail`.`encustomer_refillprice`, `tbl_invoice_detail`.`discount_price` FROM `tbl_invoice` LEFT JOIN `tbl_invoice_detail` ON `tbl_invoice`.`idtbl_invoice`=`tbl_invoice_detail`.`tbl_invoice_idtbl_invoice` WHERE `tbl_invoice`.`status`=1 AND `tbl_invoice`.`paymentcomplete`=0 AND `tbl_invoice`.`tbl_customer_idtbl_customer`='$customerID'";
                $resultinvoicelist =$conn-> query($sqlinvoicelist);
                ...
                $sqlinvoicelist="SELECT SUM(CASE WHEN `tbl_invoice_detail`.`tbl_product_idtbl_product` = 1 THEN `tbl_invoice_detail`.`refillqty` ELSE 0 END) AS refillqty, SUM(`tbl_invoice`.`nettotal`) AS nettotal,`tbl_invoice_detail`.`encustomer_refillprice`, `tbl_invoice_detail`.`discount_price` FROM `tbl_invoice` LEFT JOIN `tbl_invoice_detail` ON `tbl_invoice`.`idtbl_invoice`=`tbl_invoice_detail`.`tbl_invoice_idtbl_invoice` WHERE `tbl_invoice`.`status`=1 AND `tbl_invoice`.`paymentcomplete`=0 AND `tbl_invoice`.`tbl_customer_idtbl_customer`='$customerID' AND `tbl_invoice`.`date` BETWEEN '$validfrom' AND '$validto'";
                $resultinvoicelist =$conn-> query($sqlinvoicelist);
                $rowinvoicelist = $resultinvoicelist-> fetch_assoc();
