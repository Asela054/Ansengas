    ...
    <tbody>
        ...
        <tr>
            ...
            for($i=$first_week;$i<=$last_week;$i++){
                $sqllastbillavg="SELECT SUM(`newqty`+`refillqty`+`emptyqty`) AS `sumqty` FROM `tbl_invoice_detail` WHERE `status`=1 AND `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `status`=1 AND `tbl_customer_idtbl_customer`='$customerID' AND WEEK(`date`)='$i')";
