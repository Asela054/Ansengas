...
$sqlinvoicedetail="SELECT `tbl_product`.`product_name`, `tbl_invoice_detail`.`newqty`, `tbl_invoice_detail`.`refillqty`, `tbl_invoice_detail`.`trustqty`, `tbl_invoice_detail`.`returnqty`, `tbl_invoice_detail`.`newrefillprice`, `tbl_invoice_detail`.`newsaleprice` FROM `tbl_invoice_detail` LEFT JOIN `tbl_product` ON `tbl_product`.`idtbl_product`=`tbl_invoice_detail`.`tbl_product_idtbl_product` WHERE `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`='$recordID' AND `tbl_invoice_detail`.`status`=1";
...
<div class="row mt-3">
    <div class="col-12">
        <table class="table table-striped table-bordered table-black bg-transparent table-sm w-100 tableprint text-center">
            ...
            <tbody>
                <?php 
                    while($rowinvoicedetail=$resultinvoicedetail->fetch_assoc()){
                        $totrefill=$rowinvoicedetail['refillqty']*$rowinvoicedetail['newrefillprice'];
                        $tottrust=$rowinvoicedetail['trustqty']*$rowinvoicedetail['newrefillprice'];
                        $totnew=$rowinvoicedetail['newqty']*$rowinvoicedetail['newsaleprice'];
            ...
            <tfoot>
                <tr>
                    <th colspan="6" class="text-left">Net Total</th>
