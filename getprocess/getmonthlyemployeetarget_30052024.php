    ...
    <tbody>
        ...
        <tr>
            ...
            foreach($productarray as $rowproductarray){
                $employeeID = $row['idtbl_employee'];
                $productID = $rowproductarray->productID;
                $sqlavaqty = "SELECT SUM(`targettank`) AS `targettank`,SUM(`targetcomplete`) AS `targetcomplete` FROM `tbl_employee_target` WHERE `status` = 1 AND `tbl_employee_idtbl_employee` = '$employeeID' AND `tbl_product_idtbl_product` = '$productID'";
                $resultavaqty = $conn->query($sqlavaqty);
                $rowavaqty = $resultavaqty->fetch_assoc();
            ?>
