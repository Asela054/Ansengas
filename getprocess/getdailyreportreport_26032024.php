...
<?php 
    ...
    $sqldaily="SELECT `tbl_invoice`.`idtbl_invoice`, `tbl_invoice`.`tax_invoice_num`, `tbl_invoice`.`date`, `tbl_invoice`.`nettotal`, `tbl_invoice_detail`.`refillqty`, `tbl_invoice_detail`.`encustomer_refillprice`, `tbl_invoice_detail`.`tbl_product_idtbl_product`, `tbl_invoice_detail`.`discount_price`, `tbl_customer`.`name` AS `cusname`, `tbl_customer`.`idtbl_customer`, `tbl_customer`.`discount_status`, `tbl_employee`.`name` AS `refname`, `tbl_vehicle`.`vehicleno`, `tbl_area`.`area` FROM `tbl_invoice` LEFT JOIN `tbl_invoice_detail` ON `tbl_invoice`.`idtbl_invoice`=`tbl_invoice_detail`.`tbl_invoice_idtbl_invoice` LEFT JOIN `tbl_customer` ON `tbl_customer`.`idtbl_customer`=`tbl_invoice`.`tbl_customer_idtbl_customer` LEFT JOIN `tbl_employee` ON `tbl_employee`.`idtbl_employee`=`tbl_invoice`.`ref_id` LEFT JOIN `tbl_vehicle_load` ON `tbl_vehicle_load`.`idtbl_vehicle_load`=`tbl_invoice`.`tbl_vehicle_load_idtbl_vehicle_load` LEFT JOIN `tbl_vehicle` ON `tbl_vehicle`.`idtbl_vehicle`=`tbl_vehicle_load`.`lorryid` LEFT JOIN `tbl_area` ON `tbl_area`.`idtbl_area`=`tbl_invoice`.`tbl_area_idtbl_area` WHERE `tbl_invoice`.`date`='$validfrom'  AND `tbl_vehicle_load`.`lorryid`='$lorry' AND `tbl_invoice`.`status`=1";
    $resultdaily =$conn-> query($sqldaily); 
    ...
    <table class="table table-striped table-bordered table-sm text-center">
        ...
        <tbody>
            ...
            if($resultdaily->num_rows>0){while($rowdaily = $resultdaily-> fetch_assoc()){ 
                ...
                $productID=$rowdaily['tbl_product_idtbl_product'];

                $refillqty=$rowdaily['refillqty'];
                $refill_price=$rowdaily['encustomer_refillprice'];
                $discount_price=$rowdaily['discount_price'];
