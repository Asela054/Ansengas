...
?>
        ...
        <tr>
            ...
            foreach ($products as $product_id => $product_name) :
                ...
                $sqlsaleinfo = "SELECT SUM(((`newprice` + (`newprice` * $vatamount / 100)) * `newqty`) + ((`encustomer_newprice` + (`encustomer_newprice` * $vatamount / 100)) * `newqty`)) AS `newsum`, SUM(((`refillprice` + (`refillprice` * $vatamount / 100)) * `refillqty`) + ((`encustomer_refillprice` + (`encustomer_refillprice` * $vatamount / 100)) * `refillqty`)) AS `refillsum` , SUM(`emptyqty`) AS `emptyqty` FROM `tbl_invoice_detail` WHERE `status`=1 AND `tbl_product_idtbl_product`='$product_id' AND `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `status`=1 AND `tbl_customer_idtbl_customer`='$customerID' AND `date` BETWEEN '$validfrom' AND '$validto')";
                $resultsaleinfo = $conn->query($sqlsaleinfo);
                $rowsaleinfo = $resultsaleinfo->fetch_assoc();
            ?>
                <td class="text-center"><?php echo $rowsaleinfo['refillsum'] != 0 ? number_format($rowsaleinfo['refillsum'], 2) : ''; ?></td>
            <?php endforeach; ?>
            <?php 
            foreach ($products as $product_id => $product_name) :
                $sqlvat = "SELECT `idtbl_vat_info`, `vat` FROM `tbl_vat_info` ORDER BY `idtbl_vat_info` DESC LIMIT 1";
                ...
                $rowsaleinfo = $resultsaleinfo->fetch_assoc();
            ?>
                <td class="text-center"><?php echo $rowsaleinfo['newsum'] != 0 ? number_format($rowsaleinfo['newsum'], 2) : ''; ?></td>
            <?php endforeach; ?>
        </tr>
<?php 
    }
