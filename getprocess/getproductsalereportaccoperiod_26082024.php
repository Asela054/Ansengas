...
foreach ($period as $key => $value) {
    ...
    foreach($productarray as $rowproductarray){ 
        ...
        $sqlsalecount="SELECT SUM(`newqty`) AS `newqty`, SUM(`refillqty`) AS `refillqty`, SUM(`emptyqty`) AS `emptyqty`, SUM(`trustqty`) AS `trustqty` FROM `tbl_invoice_detail` WHERE `status`=1 AND `tbl_product_idtbl_product`='$productID' AND `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `status`=1 AND `date`='$days')";
        ...
        if(!empty($rowsalecount['emptyqty'])){
            $emptysalecount=$rowsalecount['emptyqty'];
        }
        if(!empty($rowsalecount['trustqty'])){
            $trustsalecount=$rowsalecount['trustqty'];
        }

        $totsalecount=$newsalecount+$refillsalecount+$emptysalecount+$trustsalecount;
