...
$sql = "SELECT 
            ...
            DAY(LAST_DAY(CURDATE())) - COALESCE(total_holidays_in_month, 0) AS working_days_in_month,
            (DAY(CURDATE()) - COALESCE(holidays_until_today, 0)) AS working_days_completed,
            DAY(LAST_DAY(CURDATE())) - COALESCE(total_holidays_in_month, 0) - (DAY(CURDATE()) - COALESCE(holidays_until_today, 0)) AS balance_days,
            FORMAT(SUM(et.targettank) / (DAY(LAST_DAY(CURDATE())) - COALESCE(total_holidays_in_month, 0)), 2) AS target_average,
            ROUND(SUM(et.targettank) / (DAY(CURDATE()) - COALESCE(holidays_until_today, 0)), 2) AS balance_complete_average,
            p.product_name, 
            et.targettank
        FROM 
        ...
        LEFT JOIN 
            ...
            GROUP BY MONTH(date)) AS holidays_in_month ON MONTH(CURDATE()) = holidays_in_month.month
        LEFT JOIN 
            (SELECT MONTH(date) AS month, COUNT(*) AS holidays_until_today
            FROM tbl_month_holiday
            WHERE MONTH(date) = MONTH(CURDATE()) AND date <= CURDATE()
            GROUP BY MONTH(date)) AS holidays_until_today ON MONTH(CURDATE()) = holidays_until_today.month
        GROUP BY 
            et.idtbl_employee_target, et.month";
...
// Function to update the feq_no of a customer and reorder the rest
function updateFeqNo(&$customers, $name, $newFeqNo) {
    if (!isset($customers[$name])) {
        throw new Exception("Customer $name not found.");
    }

    $oldFeqNo = $customers[$name];

    // If the new feq_no is the same as the old one, no need to update
    if ($oldFeqNo == $newFeqNo) {
        return;
    }

    // Update the feq_no of the specified customer
    $customers[$name] = $newFeqNo;

    // Reorder the rest of the customers
    foreach ($customers as $key => &$value) {
        if ($key == $name) {
            continue; // Skip the customer we just updated
        }

        if ($newFeqNo > $oldFeqNo) {
            // If the new feq_no is greater than the old one, decrement feq_no for customers in between
            if ($value > $oldFeqNo && $value <= $newFeqNo) {
                $value--;
            }
        } else {
            // If the new feq_no is less than the old one, increment feq_no for customers in between
    ...
    // Sort the array by feq_no to maintain order
    asort($customers);
}

// Example usage:

// Change feq_no of Name5 to 1
updateFeqNo($customers, 'Name5', 1);
print_r($customers);

// Change feq_no of Name6 to 3
updateFeqNo($customers, 'Name6', 3);
print_r($customers);

// Change feq_no of Name1 to 4
updateFeqNo($customers, 'Name1', 4);
print_r($customers);
...
?>                     
                                ...
                                <table class="table table-striped table-bordered table-sm small">
                                    <thead  class="bg-danger-soft">
                                        <tr>
                                            ...
                                            <th>Product</th>
                                            <th>Target</th>
                                            <th>Complete no Working days</th>
                                            <th>Complete day</th>
                                            <th>Balance Days</th>
                                            <th>Average</th>
                                            <th>Balance complete Average</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    <tbody>
                                        <?php
                                         if ($result->num_rows > 0) {
                                            ...
                                            <tr>
                                                <td><?php echo $rowinfo['employee_name']; ?></td>
                                                <td><?php echo $rowinfo['month_name']; ?></td>
                                                <td><?php echo $rowinfo['product_name']; ?></td>
                                                <td><?php echo $rowinfo['targettank']; ?></td>
                                                <td><?php echo $rowinfo['working_days_in_month']; ?></td>
                                                <td><?php echo $rowinfo['working_days_completed'] ?></td>
                                                <td><?php echo $rowinfo['balance_days'] ?></td>
                                                <td><?php echo $rowinfo['target_average'] ?>%</td>
                                                <td><?php echo $rowinfo['balance_complete_average'] ?>%</td>
                                            </tr>
                                        <?php
                                            }
                                         } else {
                            ...
                            <!-- <div class="col-7"> -->
                                <!-- <h6 class="small title-style"><span>Sales Outstanding</span></h6>
                                <table class="table table-striped table-bordered table-sm small">
                                    <thead>
                                        <tr>
                                            <th>Sales Excecutive</th>
