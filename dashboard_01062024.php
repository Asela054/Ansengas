...
for($i=1; $i<=$dayscount; $i++){
    ...
    if($i<=date('d')){
        ...
        foreach($arrayproduct as $rowproduct){
            ...
            $invdate=date('Y-m-').$i;
            $sqlsaleproduct = "SELECT SUM(`newqty`) AS `newqty`, SUM(`refillqty`) AS `refillqty` FROM `tbl_invoice_detail` WHERE `tbl_invoice_idtbl_invoice` IN (SELECT `idtbl_invoice` FROM `tbl_invoice` WHERE `date`='$invdate' AND `status`=1) AND `status`=1 AND `tbl_product_idtbl_product`='$rowproduct' HAVING SUM(`newqty`) + SUM(`refillqty`) <> 0";
