...
while ($row = mysqli_fetch_array($res)) {
    ...
    $sqlinvoiceinfo = "SELECT `tbl_invoice_detail`.*, `tbl_product`.`product_name` FROM `tbl_invoice_detail` LEFT JOIN `tbl_product` ON `tbl_product`.`idtbl_product`=`tbl_invoice_detail`.`tbl_product_idtbl_product` WHERE `tbl_invoice_detail`.`status`=1 AND `tbl_invoice_detail`.`tbl_invoice_idtbl_invoice`='$invoiceID'";
