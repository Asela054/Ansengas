...
$sqlloadinfo = "SELECT `refid` FROM `tbl_vehicle_load` WHERE `idtbl_vehicle_load`='$vehicleloadID' AND `status`=1";
$resultloadinfo = $con->query($sqlloadinfo);
$rowloadinfo = $resultloadinfo->fetch_assoc();

$saleexID=$rowloadinfo['refid'];

if($vat_status==1){
    $insertinvoice="INSERT INTO `tbl_invoice`(`tax_invoice_num`,`date`, `total`, `taxamount`, `nettotal`, `paymentmethod`, `paymentcomplete`, `chequesend`, `companydiffsend`, `ref_id`, `vat`, `status`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_area_idtbl_area`, `tbl_customer_idtbl_customer`, `tbl_vehicle_load_idtbl_vehicle_load`) VALUES ('$nextTaxInvoiceNum','$invoicedate','$withouttaxtotal','$taxamount','$nettotal','0','0','0','0','$saleexID','$vatvalue','1','$updatedatetime','$refID','$areaID','$customerID','$vehicleloadID')";
}else{
    $insertinvoice="INSERT INTO `tbl_invoice`(`date`, `total`, `taxamount`, `nettotal`, `paymentmethod`, `paymentcomplete`, `chequesend`, `companydiffsend`, `ref_id`, `vat`, `status`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_area_idtbl_area`, `tbl_customer_idtbl_customer`, `tbl_vehicle_load_idtbl_vehicle_load`) VALUES ('$invoicedate','$withouttaxtotal','$taxamount','$nettotal','0','0','0','0','$saleexID','$vatvalue','1','$updatedatetime','$refID','$areaID','$customerID','$vehicleloadID')";
}
...
if($con->query($insertinvoice)==true){
    ...
    $sqlinovicelist="SELECT `idtbl_invoice`, `total`, `paymentcomplete`, `tax_invoice_num`, `non_tax_invoice_num` FROM `tbl_invoice` WHERE `status`=1 AND `idtbl_invoice`='$invoiceID'";
    ...
    while($rowinovicelist = $resultinovicelist-> fetch_assoc()){
        ...
        $objinvoice->taxinvoicenum=$rowinovicelist['tax_invoice_num'];
        $objinvoice->nontaxinvoicenum=$rowinovicelist['non_tax_invoice_num'];

        array_push($arrayinvoicelist, $objinvoice);
    }

    $con->commit();
    $response = array("code" => '200', "message" => 'Update Complete', "invoiveid" => $arrayinvoicelist);
