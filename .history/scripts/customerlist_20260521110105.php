...
$columns = array(
	...
	array( 'db' => '`u`.`feqno`', 'dt' => 'feqno', 'field' => 'feqno' ),
	array( 'db' => '`u`.`alias_name`', 'dt' => 'alias_name', 'field' => 'alias_name' ),
...
$joinQuery = "FROM `tbl_customer` AS `u` LEFT JOIN `tbl_area` AS `ud` ON (`ud`.`idtbl_area` = `u`.`tbl_area_idtbl_area`)";
...
$extraWhere = ($showDeactivated ? "`u`.`status` = 2" : "`u`.`status` IN (1, 2, 5)") . $typeFilter;
