...
<script>
    $(document).ready(function() {
            ...
            "columns": [
                {
                    ...
                    "render": function(data, type, full) {
                        return 'CN-'+full['idtbl_purchase_return'];     
                    }
                },
                {
                    "data": "invoicenum"
        ...
        function updateTotalForRow(row) {
            ...
            var refillPrice = parseFloat(row.find('td:eq(5)').text()) || 0;

            var emptyPricewithoutvat = parseFloat(row.find('td:eq()').text()) || 0;
            var refillPricewithoutvat = parseFloat(row.find('td:eq(6)').text()) || 0;
