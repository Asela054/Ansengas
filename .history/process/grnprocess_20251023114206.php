...
if ($result->num_rows > 0) {
    ...
    if ($totqty > 0) {
        $updatestock = "UPDATE `tbl_stock` 
                        SET `fullqty` = (`fullqty` + '$newqty' + '$refillqty' + '$trustqty' + '$saftyqty'), 
                            `emptyqty` = (`emptyqty` + '$emptyqty')  
                        WHERE `tbl_product_idtbl_product` = '$product'";
        $conn->query($updatestock);
    } elseif ($emptyqty > 0) {
        $updatestock = "UPDATE `tbl_stock` 
                        SET `emptyqty` = (`emptyqty` + '$emptyqty') 
                        WHERE `tbl_product_idtbl_product` = '$product'";
        $conn->query($updatestock);
    }

} else {
    // No stock record yet — insert it
    $insertStockQuery = "INSERT INTO tbl_stock 
        (fullqty, emptyqty, damageqty, status, tbl_user_idtbl_user, tbl_product_idtbl_product)
        VALUES ('$totqty', '$emptyqty', '0', '1', '$userID', '$product')";
    $conn->query($insertStockQuery);

    // For first record, previous stock is 0
    $prevfullstock = 0;
    $prevemptystock = 0;
}

// Calculate after-insertion totals
...
VALUES ('1','$grndate','$prevfullstock','$totqty','$avafullstock',
