...
$query = "
    SELECT tc.*, c.type as customer_type, c.name as customer_name, c.owner_name, c.address, c.phone, c.nic,e.name as employee_name
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    if ($detail_result) {
        while ($row = mysqli_fetch_assoc($detail_result)) {
            ...
            // Calculate cylinder quantities based on product ID
...
$commercial_checked = $customer_type == 1 ? '✓' : ''; // 1 = Commercial
$dealer_checked = $customer_type == 2 ? '✓' : ''; // 2 = Dealer
$distributor_checked = $customer_type == 3 ? '✓' : ''; // 3 = Distributor
$other_checked = (!in_array($customer_type, [1, 2, 3])) ? '✓' : ''; // Other types
