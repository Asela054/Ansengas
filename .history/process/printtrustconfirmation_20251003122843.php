...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE tc.idtbl_trust_confirmation = ?
    ");
    $stmt->execute([$record_id]);
    $trustData = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE tcd.tbl_trust_confirmation_idtbl_trust_confirmation = ?
    ");
    $stmt->execute([$record_id]);
    $cylinderDetails = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Initialize cylinder quantities
    $stock_12_5 = 0;
    $stock_37_5 = 0;

    // Calculate cylinder quantities based on product size
    foreach ($cylinderDetails as $detail) {
        if ($detail['size'] == '12.5kg') {
            $stock_12_5 = $detail['qty'];
        } elseif ($detail['size'] == '37.5kg') {
            $stock_37_5 = $detail['qty'];
        }
    }
    ...
    // Determine customer type checkboxes
    $customer_type = strtolower($trustData['customer_type'] ?? '');
    $commercial_checked = $customer_type == 'commercial' ? '✓' : '';
    $dealer_checked = $customer_type == 'dealer' ? '✓' : '';
    $distributor_checked = $customer_type == 'distributor' ? '✓' : '';
    $other_checked = ($customer_type != 'commercial' && $customer_type != 'dealer' && $customer_type != 'distributor') ? '✓' : '';
        ...
        <tr>
            ...
            <td style="border: 1px solid #000; border-left: none; padding: 3px; height: 18px;">
                <table style="width:100%; border-collapse: collapse;">
                    <tr>
                        <td style="width:25%; border:none; padding:0; text-align:center;">' . $commercial_checked . ' Commercial</td>
                        <td style="width:25%; border:none; padding:0; text-align:center;">' . $dealer_checked . ' Dealer</td>
