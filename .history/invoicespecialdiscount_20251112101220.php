...
$sql = "SELECT c.`idtbl_customer`, c.`name` FROM `tbl_customer` c INNER JOIN `tbl_customer_product_special` cs ON c.`idtbl_customer` = cs.`tbl_customer_idtbl_customer`WHERE c.`status` = 1 
        GROUP BY c.`idtbl_customer`, c.`name`
        ORDER BY c.`name` ASC";
