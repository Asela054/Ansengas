...
<div id="purchaseview">
	<div class="modal fade" id="porderviewmodal" data-backdrop="static" data-keyboard="false" tabindex="-1"
		...
		<div class="modal-dialog modal-dialog-centered modal-lg">
			...
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="staticBackdropLabel">View Trust Confirmation</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
...
<!-- Modal -->
<div class="modal fade" id="uploadpdfModal" data-backdrop="static" data-keyboard="false" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-sm">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><i class="fas fa-file-pdf"></i> UPLOAD PDF</h4>
      ...
      <div class="modal-body">
        <form id="pdfUploadForm" enctype="multipart/form-data">
          ...
          <div class="form-group text-right mt-2">
            <button type="submit" class="btn btn-primary btn-sm px-4" id="submitBtnPdf">
...
$('#pdfUploadForm').on('submit', function(e) {
  ...
  $.ajax({
    url: 'process/upload_to_drive.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
    dataType: 'json',
    beforeSend: function() {
      $('#submitBtnPdf').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Uploading...');
    },
    success: function(res) {
      $('#submitBtnPdf').prop('disabled', false).html('<i class="fas fa-upload"></i> Upload');
...
<script>
    $(document).ready(function() {
        ...
        $('#dataTable').DataTable( {
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, full) {
                        var button = '';
                        button += '<button class="btn btn-outline-orange btn-sm btnuploadpdf mr-1" id="' + full['idtbl_trust_confirmation'] + '" data-toggle="tooltip" data-placement="bottom" title="Upload Trust Confirmation PDF"><i class="fas fa-upload"></i></button>';
                        button += '<a href="process/printtrustconfirmation.php?record=' + full['idtbl_trust_confirmation'] + '" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Print Trust Confirmation" class="btn btn-outline-secondary btn-sm mr-1 ';
                        if (editcheck != 1) {
                            button += 'd-none';
                        }
                        ...
                        button += '<a href="process/statustrustconfirmation.php?record=' + full['idtbl_trust_confirmation'] + '&type=3" onclick="return delete_confirm()" target="_self" class="btn btn-outline-danger btn-sm mr-1 ';
        ...
        $('#dataTable tbody').on('click', '.btnuploadpdf', function () {
            var id = $(this).attr('id');
            $("#hiddenid").val(id);
        ...
        $('#dataTable tbody').on('click', '.btnview', function () {
            ...
            $.ajax({
                ...
                url: 'getprocess/trustconfirmationview.php',
    ...
    $('#btnordercreate').click(function () {
        ...
        $('#btncreateorder').click(function () {
            ...
            $('#tableBody tr').each(function () {
                ...
                var fullQty = row.find('.full-qty-input').val().trim();
                var comment = row.find('textarea[name="comment"]').val().trim();

                if (productId && fullQty !== '') {
                    orderDetails.push({
                        productId: productId,
                        fullQty: fullQty,
                        comment: comment
            ...
            $.ajax({
                url: 'process/trustconfirmationprocess.php',
                type: 'POST',
                dataType: 'json',
                data: data,
