...
// Build the main query to get invoice details with customer and product information
$sql = "SELECT 
            i.idtbl_invoice,
            i.date,
            i.tax_invoice_num,
...
if($result && $result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $total_qty = $row['refillqty'] + $row['emptyqty'] + $row['trustqty'] - $row['trustreturnqty'];
        // $total_qty = $row['newqty'] + $row['refillqty'] + $row['emptyqty'] + $row['trustqty'] - $row['trustreturnqty'];
        
        // Use special price if available, otherwise use discount price from invoice detail
