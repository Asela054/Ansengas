        ...
        <table
            ...
            <?php
                foreach ($invoiceDetails as $rowinvoicedetail) {
                    // Check for empty prices and use alternative customer type prices if needed
                    $newPrice = $rowinvoicedetail['newprice'];
                    $refillPrice = $rowinvoicedetail['refillprice'];
                    $emptyPrice = $rowinvoicedetail['emptyprice'];
                    $encusNewPrice = $rowinvoicedetail['encustomer_newprice'];
                    $encusRefillPrice = $rowinvoicedetail['encustomer_refillprice'];
                    $encusEmptyPrice = $rowinvoicedetail['encustomer_emptyprice'];
                    
                    // If current customer type is 2 but prices are empty, use type 1 prices
                    if ($cusType == 2 && (empty($newPrice) || empty($refillPrice) || empty($emptyPrice))) {
                        $newPrice = $rowinvoicedetail['encustomer_newprice'];
                        $refillPrice = $rowinvoicedetail['encustomer_refillprice'];
                        $emptyPrice = $rowinvoicedetail['encustomer_emptyprice'];
                    }
                    // If current customer type is 1 but encustomer prices are empty, use type 2 prices
                    else if ($cusType == 1 && (empty($encusNewPrice) || empty($encusRefillPrice) || empty($encusEmptyPrice))) {
                        $encusNewPrice = $rowinvoicedetail['newprice'];
                        $encusRefillPrice = $rowinvoicedetail['refillprice'];
                        $encusEmptyPrice = $rowinvoicedetail['emptyprice'];
                    }

                    // Check if cusType is equal to 1
                    if ($cusType == 1) {
                        $vatNew = $encusNewPrice * $rowvat['vat'] / 100;
                        $vatRefill = $encusRefillPrice * $rowvat['vat'] / 100;
                        $vatEmpty = $encusEmptyPrice * $rowvat['vat'] / 100;
                    } else {
                        $vatNew = $rowvat['vat'] * $newPrice / 100;
                        $vatRefill = $rowvat['vat'] * $refillPrice / 100;
                        $vatEmpty = $rowvat['vat'] * $emptyPrice / 100;
                    }

                    // Calculate total prices including VAT
                    $totalWithVAT = number_format(
                        ($rowinvoicedetail['newqty'] * ($newPrice + $vatNew))
                        + ($rowinvoicedetail['refillqty'] * ($refillPrice + $vatRefill))
                        + ($rowinvoicedetail['emptyqty'] * ($emptyPrice + $vatEmpty))
                        + ($rowinvoicedetail['trustqty'] * ($refillPrice + $vatRefill))
                        + ($rowinvoicedetail['trustreturnqty'] * 0),
                        2
                    );

                    $totalWithVATencus = number_format(
                        ($rowinvoicedetail['newqty'] * ($encusNewPrice + $vatNew))
                        + ($rowinvoicedetail['refillqty'] * ($encusRefillPrice + $vatRefill))
                        + ($rowinvoicedetail['emptyqty'] * ($encusEmptyPrice + $vatEmpty))
                        + ($rowinvoicedetail['trustqty'] * ($encusRefillPrice + $vatRefill))
                        + ($rowinvoicedetail['trustreturnqty'] * 0),
                        2
                    );
                    ?>
                    ...
                    <tr>
                        ...
                        <td class="text-right"><?php echo number_format(($cusType == 1) ? $encusNewPrice + $vatNew : ($newPrice + $vatNew), 2); ?></td>
                        <td class="text-right"><?php echo number_format(($cusType == 1) ? $encusRefillPrice + $vatRefill : ($refillPrice + $vatRefill), 2); ?></td>
                        <td class="text-right"><?php echo number_format(($cusType == 1) ? $encusEmptyPrice + $vatEmpty : ($emptyPrice + $vatEmpty), 2); ?></td>
                    ...
                    <table class="table table-striped table-bordered table-black bg-transparent table-sm w-100 text-center">
                        ...
                        foreach ($invoiceDetails as $rowinvoicedetail) {
                            ...
                            <tr>
                                ...
                                <td class="text-right">' . number_format((($cusType == 1) ? $encusNewPrice : $newPrice), 2) . '</td>
                                <td class="text-right">' . number_format((($cusType == 1) ? $encusRefillPrice : $refillPrice), 2) . '</td>
                                <td class="text-right">' . number_format((($cusType == 1) ? $encusEmptyPrice : $emptyPrice), 2). '</td>
                                <td class="text-right">' . $rowvat['vat'] . '%</td>
                                <td class="text-right">' . number_format(($cusType == 1) ? $encusNewPrice + $vatNew : ($newPrice + $vatNew), 2) . '</td>
                                <td class="text-right">' . number_format(($cusType == 1) ? $encusRefillPrice + $vatRefill : ($refillPrice + $vatRefill), 2) . '</td>
                                <td class="text-right">' . number_format(($cusType == 1) ? $encusEmptyPrice + $vatEmpty : ($emptyPrice + $vatEmpty), 2) . '</td>
                            </tr>';
                        }
