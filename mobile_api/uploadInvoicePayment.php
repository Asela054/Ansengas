...
if ($con->query($insertpayment)) {
    ...
    foreach ($tblData as $rowtabledata) {
        ...
        if ($invpayamount > 0) {
            ...
            $sqlsumpayment="SELECT SUM(`payamount`) AS `netpayment` FROM `tbl_invoice_payment_has_tbl_invoice` WHERE `tbl_invoice_idtbl_invoice`='$invno'";
            $resultsumpayment = $con->query($sqlsumpayment);
