...
$columns = array(
	...
	array( 'db' => '`i`.`tax_invoice_num`', 'dt' => 'tax_invoice_num', 'field' => 'tax_invoice_num' ),
...
$joinQuery = "FROM `tbl_invoice_payment` AS `u` LEFT JOIN `tbl_invoice_payment_has_tbl_invoice` AS `ua` ON `ua`.`tbl_invoice_payment_idtbl_invoice_payment` = `u`.`idtbl_invoice_payment` LEFT JOIN `tbl_invoice` AS `i` ON `i`.`idtbl_invoice` = `ua`.`tbl_invoice_idtbl_invoice`";
