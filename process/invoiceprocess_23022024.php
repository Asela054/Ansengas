...
$sql = "SELECT `type` FROM `tbl_customer` WHERE `idtbl_customer`='$customerID'";
...
if($conn->query($insertinvoice)==true){
    ...
    foreach ($orderDetails as $detail) {
        ...
        $newPrice = $refillPrice = $emptyPrice = $encustomernewPrice = $encustomerrefillPrice = $encustomeremptyPrice = 0;
        ...
        if ($newQty > 0 || $refillQty > 0 || $emptyQty > 0 || $trustQty > 0 || $trustreturnQty > 0) {
            $insertinvoicedetail = "INSERT INTO `tbl_invoice_detail` (`newqty`, `refillqty`, `emptyqty`, `trustqty`, `trustreturnqty`, `newprice`, `refillprice`, `emptyprice`, `encustomer_newprice`, `encustomer_refillprice`, `encustomer_emptyprice`, `status`, `updatedatetime`,  `tbl_user_idtbl_user`, `tbl_product_idtbl_product`,  `tbl_invoice_idtbl_invoice`)  VALUES ('$newQty', '$refillQty', '$emptyQty', '$trustQty', '$trustreturnQty', '$newPrice', '$refillPrice', '$emptyPrice', '$encustomernewPrice','$encustomerrefillPrice','$encustomeremptyPrice', '1', '$updatedatetime', '$userID', '$product', '$invoiceID')";
