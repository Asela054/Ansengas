...
if($conn->query($insertinvoice)==true){
    ...
    foreach ($orderDetails as $detail) {
        ...
        if (!empty($newQty) || !empty($refillQty) || !empty($emptyQty)) { $newPrice = !empty($newQty) ? $unitprice : null; $refillPrice = !empty($refillQty) ? $refillprice : null;$emptyPrice = !empty($emptyQty) ? $emptyprice : null; $discountedPrice = !empty($refillQty) ? $discountedprice : null;
        
            $insertinvoicedetail = "INSERT INTO `tbl_invoice_detail` (`newqty`, `refillqty`, `emptyqty`, `newprice`, `refillprice`, `emptyprice`, `discountedprice`, `status`, `updatedatetime`,  `tbl_user_idtbl_user`, `tbl_product_idtbl_product`,  `tbl_invoice_idtbl_invoice`)  VALUES ('$newQty', '$refillQty', '$emptyQty', '$newPrice', '$refillPrice', '$emptyPrice', '$discountedPrice', '1', '$updatedatetime', '$userID', '$product', '$invoiceID')";
        
            $conn->query($insertinvoicedetail);
        }

        $totqty=$newQty+$refillQty+$emptyQty;
