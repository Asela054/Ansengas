...
if($conn->query($insertinvoice)==true){
    ...
    foreach ($orderDetails as $detail) {
        ...
        } elseif (!empty($trustQty)) {
            $refillPrice = $refillprice;
        } elseif (!empty($trustreturnQty)) {
            $refillPrice = $refillprice;
        }
    
        $emptyPrice = !empty($emptyQty) ? $emptyprice : null;
        $discountedPrice = !empty($refillQty) ? $discountedprice : null;
    
        if (!empty($newQty) || !empty($refillQty) || !empty($emptyQty) || !empty($trustQty) || !empty($trustreturnQty)) {
