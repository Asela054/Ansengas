...
$record=$_GET['record'];
$type=$_GET['type'];
$cancelReason = isset($_GET['cancel_reason']) ? $_GET['cancel_reason'] : '';
...
$sql="UPDATE `tbl_invoice` SET `status`='$value', `cancel_reason`='".$conn->real_escape_string($cancelReason)."', `tbl_user_idtbl_user`='$userID', `updatedatetime`='$updatedatetime' WHERE `idtbl_invoice`='$record'";
if($conn->query($sql)==true){
    ...
    while($rowcheckdetail = $resultcheckdetail-> fetch_assoc()){
        ...
        $emptyqty=$rowcheckdetail['emptyqty'];
        $trustqty=$rowcheckdetail['trustqty'];
        $returnqty=$rowcheckdetail['trustreturnqty'];

        $invdetailID=$rowcheckdetail['idtbl_invoice_detail'];

        $totqty=$newqty+$refillqty+$trustqty;
        $totemptyqty=$emptyqty-($refillqty + $returnqty);

       echo $updatestock="UPDATE `tbl_vehicle_load_detail` SET  qty = qty + $totqty, emptyqty = emptyqty + $totemptyqty WHERE `tbl_product_idtbl_product`='$productID' AND `tbl_vehicle_load_idtbl_vehicle_load`='$vehicleloadID'";
