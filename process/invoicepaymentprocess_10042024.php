...
if($conn->query($insertpayment)==true){
    ...
    foreach($tblData as $rowtabledata){
        ...
        if($invpayamount>0){
            $updateinvoice="UPDATE `tbl_invoice` SET `paymentcomplete`='$paymentcompletestatus',`excess_payment`='0',`updatedatetime`='$updatedatetime',`tbl_user_idtbl_user`='$userID' WHERE `idtbl_invoice`='$invoiceID'";
