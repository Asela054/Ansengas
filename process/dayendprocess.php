...
if($recordOption==1){
    $sqlcheck = "SELECT SUM(tbl_stock.fullqty*tbl_product.refillprice) AS stock_close_value FROM tbl_stock INNER JOIN tbl_product ON tbl_stock.tbl_product_idtbl_product=tbl_product.idtbl_product WHERE tbl_stock.status=1 AND tbl_stock.fullqty>0";  //unitprice--->refillprice
