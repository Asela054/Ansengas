...
$emptysaleprice = $_POST['emptysaleprice'];
...
if($recordOption==1){
    $query = "INSERT INTO `tbl_product`(`product_code`, `product_name`, `size`, `newprice`, `refillprice`, `emptyprice`, `newsaleprice`, `refillsaleprice`, `emptysaleprice`, `status`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_product_category_idtbl_product_category`) Values ('$productcode','$product_name','','$unitprice','$refillprice','$emptyprice','$newsaleprice','$refillsaleprice','$emptysaleprice','1','$updatedatetime','$userID','$category')";
    ...
    $query = "UPDATE `tbl_product` SET `product_code`='$productcode',`product_name`='$product_name',`newprice`='$unitprice',`refillprice`='$refillprice',`emptyprice`='$emptyprice',`newsaleprice`='$newsaleprice',`refillsaleprice`='$refillsaleprice',`emptysaleprice`='$emptysaleprice',`updatedatetime`='$updatedatetime',`tbl_user_idtbl_user`='$userID', `tbl_product_category_idtbl_product_category`='$category' WHERE `idtbl_product`='$recordID'";
    if($conn->query($query)==true){
        $sqlcuslist="SELECT `idtbl_customer` FROM `tbl_customer` WHERE `type` IN (2) AND `status` IN (1, 2)";
        $resultcuslist=$conn->query($sqlcuslist);
        while ($rowcuslist = $resultcuslist-> fetch_assoc()) {
