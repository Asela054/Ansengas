...
foreach ($orderDetails as $detail) {
    ...
    if (!empty($newQty) || !empty($refillQty) || !empty($trustQty) || !empty($returnQty) || !empty($saftyQty) || !empty($saftyReturnQty)) {

        $insertDetailQuery = "INSERT INTO tbl_porder_detail (`type`, `refillqty`, `returnqty`, `newqty`, `trustqty`, `saftyqty`, `saftyreturnqty`, `unitprice`, `refillprice`, `status`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_porder_idtbl_porder`, `tbl_product_idtbl_product`) 
                             VALUES ('0', '$refillQty', '$returnQty', '$newQty', '$trustQty', '$saftyQty', '$saftyReturnQty', '$unitPrice', '$refillPrice', '1', '$updatedatetime', '$userID', '$orderID', '$productId')";
